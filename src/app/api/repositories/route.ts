...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sync) {
      console.log(`[API/Repositories] SYNC: User ${session.user.id}, Page ${page}, Limit ${limit}`);
      ...
      const repos = await Promise.all(
        githubRepos.map(async (ghRepo) => {
          ...
          return Repository.findOneAndUpdate(
            ...
            { $set: repoData, $currentDate: { updatedAt: true } }, // Explicitly update timestamp
      ...
      console.log(`[API/Repositories] SYNC complete. Fetched ${githubRepos.length} from GitHub. Returning ${userSyncedRepos.length} for user DB page ${page}. Total synced for user: ${totalUserSyncedRepos}`);
