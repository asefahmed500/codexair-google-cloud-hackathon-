...
import clientPromise, { User as UserModel, connectMongoose, Account as AccountModel } from './mongodb'; // Added AccountModel
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      ...
      if (!dbUser) {
        ...
        try {
          dbUser = await UserModel.create({
            ...
            email: userEmail,
            image: user.image,
            role: "user", // Default role, also set by Mongoose schema default
            status: "active", // Default status
          });
          // console.log(`[SignIn Callback] New user created with ID: ${dbUser._id}, Role: ${dbUser.role}`);
        } catch (error) {
          console.error(`[SignIn Callback] Error creating new user for email '${userEmail}':`, error);
          return `/auth/signin?error=UserCreationError`;
        }
      } else {
      ...
      // The adapter will handle linking the account to the user.
    ...
    async jwt({ token, user, account }) {
      ...
      if (account && user && user.id) {
        ...
        if (dbUser) {
          ...
          delete token.role; delete token.status; // Keep token.id, email, name, picture if they were there from previous valid state
        }
      }
